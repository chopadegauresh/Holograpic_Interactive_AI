...
import websockets
import re
ALLOW_QUERY_LISTENING = True

clients = set()

#below one is newly added for ws server
...
def broadcast_audio(chunk):
    print("broadcast_audio called")
    for ws in list(clients):
        try:
            asyncio.run(ws.send(chunk))
...
# ================= CONFIG =================
ACCESS_KEY = "Put your Porcupine wake word access key here" #Replace with your actual access key
WAKE_WORD = "hey siri"
...
GROQ_API_KEY = os.getenv("Put your Groq API key here") #Replace with your actual GROQ API key
GROQ_URL = "*******************" #Replace with your actual GROQ API endpoint
...
def run_skill(query):
    ...
    if "date" in q or "day" in q or "today" in q or "date" in q:
    ...
    if "founder" in q or "created " in q or "made" in q or "developed" in q or "built" in q or "creator" in q or "invented" in q or "father" in q:
...
SYSTEM_PROMPT = "You are Siri, a helpful and friendly voice assistant developed by gauresh chopade. Give short, clear answers. Be conversational but concise."
        ...
        try:
            ...
            if content:
                ...
                if any(p in sentence_buffer for p in ['!', '?']):
                    ...
                    sentence_buffer = ""
                    time.sleep(1.0)   # pause between sentences to slow down online speed
        except:
            continue
...
# ================= TTS ====================
def speech_worker():
    global SPEAKING, ALLOW_QUERY_LISTENING
    while True:
        text = speech_q.get()

        ALLOW_QUERY_LISTENING = False   # 🔴 block new queries
        SPEAKING = True
        print("🔊 TTS STARTED")
        stop_event.clear()
        SPEAKING = True
        ...
        try:
            process = subprocess.Popen(
                ["piper", "--model", PIPER_MODEL, "--length_scale", "1.25", "--noise_scale", "0.667", "--output_raw"],
            ...
            if process and process.poll() is None:
                process.kill()
            SPEAKING = False
            ALLOW_QUERY_LISTENING = True    # 🟢 allow queries again
            speech_q.task_done()
            print("🔊 TTS FINISHED")
    ...
    while True:
        if MODE == "WAKE":
            ...
            if porcupine.process(pcm) >= 0:
                ...
                # 1. Send the special command to block body animation
                #broadcast_command("STATE:WAVING_ONLY")
                # This will move the mouth and body on the 'Base' layer
               # time.sleep(0.3)
                speak("Yes? Siri here")
            ...
            if not ALLOW_QUERY_LISTENING:
                time.sleep(0.05)
                continue
            query = listen_sentence(20)
            if not query:
            ...
            if follow_start is not None and time.time() - follow_start > FOLLOWUP_TIMEOUT:
                MODE = "WAKE"
                continue
